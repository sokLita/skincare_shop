              ...
              <div class="social-btns">
                ...
                >
                  ...
                  <span class="social-btn-text">
                    ...
                    <span v-else>{{ t('register.google') }}</span>
                  </span>
                </button>
...
export default {
  ...
  methods: {
    continueWithGoogle() {
      ...
      // the session cookie is scoped to the backend domain (e.g. localhost:8000).
      // Google will redirect back to the backend's /auth/google/callback, and the
      // browser will send the session cookie correctly, allowing Socialite to validate
      // the state parameter.
    ...
    socialSignup(provider) {
      // Kept for compatibility; now uses the same handler as the Google button.
      if (provider !== 'google') {
